...
import { activity, categoryStock, statusOf, type ActivityKind } from "@/data/inventory";
...
> = {
  low: { icon: TriangleAlert, wrap: "bg-warning/12 text-warning", tag: "bg-warning/12 text-warning" },
  out: { icon: PackageX, wrap: "bg-destructive/12 text-destructive", tag: "bg-destructive/12 text-destructive" },
  restock: { icon: RotateCw, wrap: "bg-primary/12 text-primary", tag: "bg-primary/12 text-primary" },
  sale: { icon: ShoppingCart, wrap: "bg-accent text-accent-foreground", tag: "bg-primary/10 text-primary" },
};
...
function Dashboard() {
  ...
  const categoryStock = Array.from(
    ...
    category,
    units: products.filter((product) => product.category === category).reduce((sum, product) => sum + product.quantity, 0),
  }));
  ...
  return (
    <DashboardShell
      title="Dashboard"
      subtitle="Welcome back! Here's your stock report"
    >
      <div className="grid gap-5 md:grid-cols-3">
        {dashboardKpis.map(({ label, value, sub, icon: Icon, tone}) => {
          ...
          <ul className="mt-4 divide-y divide-border">
            {activity.map((a) => {
              ...
              return (
                ...
                >
                  <span className={cn("grid size-9 shrink-0 place-items-center rounded-lg", s.wrap)}>
            ...
            {activity.length === 0 && (
              <li className="py-8 text-center text-sm text-muted-foreground">No recent activity yet.</li>
            )}
          </ul>
        </section>
      </div>
